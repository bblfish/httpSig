...
class Signature_Test extends munit.FunSuite :
	...
	for ex <- Seq(`§3.2`("sig1"), `§3.2`("sig1", 3, 1, 2), `§3.2`("sig1", 4, 0, 7),
			`§3.2`("sig1", 4, 5), `§4.3_second`("sig1"), `§4.3_second`("sig1", 2, 5),
			`§4.3_second`("sig1", 4, 7, 0, 1))
	do test(s"${ex.name} `Signature` header with ${ex.lspace} ${ex.rspace} spaces around sig" +
	...
	for (ex1,ex2) <- Seq(`§3.2`("sig1"), `§3.2`("sig1", 3, 1), `§3.2`("sig1", 4, 0, 7))
		.zip(Seq(`§4.3_second`("sig2", 4, 5), `§4.3_second`("sig2"), `§4.3_second`("sig2", 2, 5, 0, 5)))
	do test(s"${ex1.name} spaced ${ex1.lspace}-${ex1.rspace} and ${ex1.`before=space`}=${ex1.`after=space`}"+
		...
		if ex1.illegalSpace || ex2.illegalSpace
		then assert(found.isEmpty && ex2 == `§4.3_second`("sig2", 2, 5, 0, 5))
		else
			assertEquals(found.get.signatures.sigmap.values.size,2) // when it fails it fails on both
