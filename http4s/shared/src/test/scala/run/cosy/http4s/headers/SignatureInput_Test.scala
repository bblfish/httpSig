...
class SignatureInput_Test extends munit.FunSuite:
	...
	test("`§3.2` example of `Signature-Input` header") {
		...
		val expected: Option[`Signature-Input`] = SigInputs(sig1.ex(0).Sig).map(`Signature-Input`.apply)
	...
	test("`§4.1` example of `Signature-Input` header") {
		...
		val expected2: Option[`Signature-Input`] = SigInputs(sig2.ex(0).Sig).map(`Signature-Input`.apply)
	...
	test("two `Signature-Input` headers with the same name. Last overwrites first.") {
		...
		val expected3: Option[`Signature-Input`] = SigInputs(sig3.ex(1).Sig).map(`Signature-Input`.apply)
