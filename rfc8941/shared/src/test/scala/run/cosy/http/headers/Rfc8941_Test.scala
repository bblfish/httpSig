...
class Rfc8941_Test extends munit.FunSuite:
   ...
   def parseFail[T](result: Either[Parser.Error, (String, T)], msg: String = "")(implicit
       loc: munit.Location
   ): Unit =
     assert(result.isLeft, result)

   def parseFailAll[T](result: Either[Parser.Error, T], msg: String = "")(implicit
       loc: munit.Location
   ): Unit =
     assert(result.isLeft, result)
     ...
     assertEquals(
       sfList.parseAll(
         """1234.750;  n=4;f=3 , 42;magic="h2g2", foo123/456;lang=en ,
           |   ?0;sleep=?1, "No/No", :cafebabe:;enc=unicode""".stripMargin.filter(
           _ != '\n'
         ).toString
       ),
       RA(List(
         PI(SfDec("1234.750"), ListMap(Token("n") -> SfInt("4"), Token("f") -> SfInt("3"))),
   ...
   test("lists of innerList") {
     ...
     assertEquals(
       ...
       R(List(
         IL(
           List(PI(sf"foo", ListMap(Token("a") -> SfInt("1"), Token("b") -> SfInt("2")))),
